...
import { db, nowISO, getDashboardStats } from '../db';
...
export default function Treasury() {
  ...
  async function load(){
    setLoading(true);
    const [s,e]=await Promise.all([getDashboardStats(),db.expenses.orderBy('createdAt').reverse().toArray()]);
  ...
  async function saveExpense(){
    ...
    setSaving(true);
    await db.expenses.add({label:form.label,amount:Number(form.amount),category:form.category,createdAt:nowISO()});
    setForm({label:'',amount:'',category:'Divers'});
    setModal(false);await load();setSaving(false);
  }
  ...
  return (
    <div>
      <PageHeader title="Trésorerie & Comptabilité"
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:16}}>
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          <Card>
            ...
            {Object.keys(catTotals).length===0&&<div style={{color:C.sub,fontSize:13,textAlign:'center',padding:16}}>Aucune dépense</div>}
          </Card>
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <Card style={{border:`1px solid ${C.accent}25`,background:C.accentLo}}>
            ...
            <div style={{borderTop:`1px solid ${C.accentMd}`,paddingTop:12}}>
              ...
              {annualBenef>0&&<div style={{color:C.green,fontSize:11,marginTop:4,fontWeight:600}}>✓ Nisab atteint</div>}
            </div>
          </Card>
      ...
      {modal&&<Modal title="+ Nouvelle dépense" onClose={()=>setModal(false)}>
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <Input label="Description *" value={form.label} onChange={e=>setForm(f=>({...f,label:e.target.value}))} placeholder="Loyer du mois..."/>
          <Input label="Montant (DA) *" type="number" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} placeholder="15000"/>
        ...
        <div style={{display:'flex',gap:10,justifyContent:'flex-end',marginTop:20}}>
          ...
          <Btn variant="danger" onClick={saveExpense} disabled={saving}>{saving?'⏳...':'✓ Enregistrer'}</Btn>
        </div>
      </Modal>}
    </div>
  );
