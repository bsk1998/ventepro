...
const D = {
  ...
  shadow:'0 4px 24px rgba(99,102,241,0.10)', shadowLg:'0 12px 40px rgba(99,102,241,0.15)',
  R:16, Rs:10,
  blue:'#3B82F6',   blueLt:'#EFF6FF',   blueBd:'#BFDBFE',
  indigo:'#6366F1', indigoLt:'#EEF2FF', indigoBd:'#C7D2FE',
  violet:'#8B5CF6', violetLt:'#F5F3FF', violetBd:'#DDD6FE',
  green:'#10B981',  greenLt:'#ECFDF5',  greenBd:'#A7F3D0',
  amber:'#F59E0B',  amberLt:'#FFFBEB',  amberBd:'#FDE68A',
  red:'#EF4444',    redLt:'#FEF2F2',    redBd:'#FECACA',
  cyan:'#06B6D4',   cyanLt:'#ECFEFF',   cyanBd:'#A5F3FC',
  pink:'#EC4899',   pinkLt:'#FDF2F8',   pinkBd:'#FBCFE8',
  txt:'#1E293B', sub:'#64748B', muted:'#94A3B8',
};
...
const G = ({ children, style, ac }) => (
  <div style={{ background:D.white, backdropFilter:'blur(12px)', WebkitBackdropFilter:'blur(12px)',
    ...
    boxShadow:D.shadow, ...style }}>{children}</div>
);

const Inp = ({ label, value, onChange, type='text', placeholder, style, color }) => (
  <div style={style}>
    {label && <div style={{ fontSize:10, fontWeight:700, color:color||D.sub, marginBottom:4,
      textTransform:'uppercase', letterSpacing:.6 }}>{label}</div>}
    <input value={value||''} onChange={onChange} type={type} placeholder={placeholder}
      style={{ width:'100%', background:color?color+'12':'#F8FAFF', border:`1.5px solid ${color?color+'40':D.indigoBd}`,
        borderRadius:D.Rs, padding:'8px 12px', color:D.txt, fontSize:13, outline:'none',
...
const Sel = ({ label, value, onChange, children, color }) => (
  <div>
    {label && <div style={{ fontSize:10, fontWeight:700, color:color||D.sub, marginBottom:4,
      textTransform:'uppercase', letterSpacing:.6 }}>{label}</div>}
    <select value={value||''} onChange={onChange}
      style={{ width:'100%', background:'#F8FAFF', border:`1.5px solid ${color?color+'40':D.indigoBd}`,
        borderRadius:D.Rs, padding:'8px 12px', color:D.txt, fontSize:13, outline:'none',
...
const Pill = ({ label, active, onClick, color }) => (
  <button onClick={onClick} style={{
    padding:'5px 14px', border:`1.5px solid ${active?color||D.blue:D.indigoBd}`,
    borderRadius:20, cursor:'pointer', fontSize:12, fontWeight:active?700:500,
    background:active?color||D.blue:D.indigoLt,
    color:active?'#fff':D.sub, transition:'all .15s', whiteSpace:'nowrap',
  }}>{label}</button>
);

const StatCard = ({ icon, label, value, color, sub }) => (
  <div style={{ background:color+'15', border:`1.5px solid ${color}30`, borderRadius:D.R,
...
function LabelPreview({ product, copies, onPrint }) {
  return (
    <div style={{ background:D.indigoLt, border:`1.5px solid ${D.indigoBd}`, borderRadius:D.R, padding:14 }}>
      ...
      {/* Étiquette simulée */}
      <div style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:8, padding:'10px 14px', marginBottom:12, textAlign:'center', boxShadow:'0 2px 8px rgba(0,0,0,.06)' }}>
      ...
      <div style={{ display:'flex', gap:8, alignItems:'center' }}>
        <Inp label="Copies" value={copies} onChange={e=>onPrint('copies',e.target.value)}
          type="number" style={{ flex:1 }} color={D.indigo}/>
...
function ProductModal({ product, onClose, onSave, suppliers, initialData }) {
  ...
  function handleLabel(action, val) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(99,102,241,0.12)',
      backdropFilter:'blur(8px)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center' }}>
      <G style={{ width:680, maxHeight:'90vh', display:'flex', flexDirection:'column',
        boxShadow:D.shadowLg }} ac={D.indigo}>

        {/* Header */}
        <div style={{ background:`linear-gradient(135deg,${D.blue},${D.indigo})`,
          padding:'14px 20px', borderRadius:`${D.R}px ${D.R}px 0 0`,
          ...
          <button onClick={onClose} style={{ background:'rgba(255,255,255,.2)', border:'none',
            borderRadius:8, width:28, height:28, color:'#fff', fontSize:18, cursor:'pointer',
            display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
        </div>

        {/* Tabs */}
        <div style={{ display:'flex', gap:4, padding:'10px 16px 0', flexShrink:0,
          borderBottom:`1px solid ${D.indigoBd}` }}>
          {TABS.map(t => (
        ...
        <div style={{ flex:1, overflowY:'auto', padding:18 }}>
          ...
          {tab==='general' && (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
              <Inp label="Nom *" value={form.name} onChange={e=>set('name',e.target.value)}
                placeholder="Nom du produit" style={{ gridColumn:'1/-1' }} color={D.blue}/>
              <Inp label="Référence" value={form.ref} onChange={e=>set('ref',e.target.value)}
                placeholder="REF-001" color={D.indigo}/>
              <Sel label="Catégorie" value={form.category} onChange={e=>set('category',e.target.value)} color={D.violet}>
                <option value="">Choisir...</option>
                {CATS.map(c=><option key={c} value={c}>{c}</option>)}
              </Sel>
              <Inp label="Stock actuel" value={form.stock} onChange={e=>set('stock',e.target.value)}
                type="number" placeholder="0" color={D.green}/>
              <Inp label="Stock minimum" value={form.minStock} onChange={e=>set('minStock',e.target.value)}
                type="number" placeholder="5" color={D.amber}/>
              <Sel label="Unité" value={form.unit} onChange={e=>set('unit',e.target.value)} color={D.cyan}>
                {UNITS.map(u=><option key={u} value={u}>{u}</option>)}
              </Sel>
              <Inp label="Date de péremption" value={form.expiry||''} onChange={e=>set('expiry',e.target.value)}
                type="date" color={D.red}/>
              <Inp label="Fournisseur" value={form.supplier||''} onChange={e=>set('supplier',e.target.value)}
                placeholder="Nom fournisseur" color={D.indigo}/>
              <div style={{ gridColumn:'1/-1' }}>
                <Inp label="Description" value={form.description||''} onChange={e=>set('description',e.target.value)}
                  placeholder="Description optionnelle..." color={D.sub}/>
              </div>
              <div style={{ gridColumn:'1/-1', display:'flex', alignItems:'center', gap:10 }}>
                <input type="checkbox" id="fav" checked={!!form.favorite} onChange={e=>set('favorite',e.target.checked)}
          ...
          {tab==='prix' && (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
              <G style={{ padding:14, gridColumn:'1/-1' }} ac={D.red}>
                <div style={{ fontSize:11, fontWeight:800, color:D.red, textTransform:'uppercase', letterSpacing:.7, marginBottom:10 }}>
                  💸 Prix d'achat
                </div>
                <Inp label="Prix d'achat (DA)" value={form.buyPrice} onChange={e=>set('buyPrice',e.target.value)}
                  type="number" placeholder="0" color={D.red}/>
              </G>

              <G style={{ padding:14, gridColumn:'1/-1' }} ac={D.blue}>
                <div style={{ fontSize:11, fontWeight:800, color:D.blue, textTransform:'uppercase', letterSpacing:.7, marginBottom:10 }}>
                  🏷️ Grille de vente
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
                  ...
                  <Inp label="Prix semi-gros (DA)" value={form.sellPriceSemiGros||''} onChange={e=>set('sellPriceSemiGros',e.target.value)}
                    type="number" placeholder="0" color={D.cyan}/>
                  <Inp label="Prix gros (DA)" value={form.sellPriceGros||''} onChange={e=>set('sellPriceGros',e.target.value)}
                    type="number" placeholder="0" color={D.violet}/>
                </div>
              </G>

              {/* Récap marges */}
              {form.buyPrice && form.sellPrice && Number(form.buyPrice)>0 && (
                <G style={{ padding:14, gridColumn:'1/-1' }} ac={D.green}>
                  <div style={{ fontSize:11, fontWeight:800, color:D.green, textTransform:'uppercase', letterSpacing:.7, marginBottom:10 }}>
                    📈 Récapitulatif marges
                  </div>
                    ...
                    {[
                      ...
                      return (
                        <div key={label} style={{ background:color+'12', border:`1px solid ${color}30`, borderRadius:D.Rs, padding:'10px 12px' }}>
                          ...
                          <div style={{ fontSize:11, color:m>0?D.green:D.red, fontWeight:700, marginTop:2 }}>
                            {m>0?'+':''}{m}% · +{fmt(Number(price)-Number(form.buyPrice))} DA
                          </div>
                        </div>
                      );
                    }).filter(Boolean)}
          ...
          {tab==='codes' && (
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <G style={{ padding:14 }} ac={D.blue}>
                <div style={{ fontSize:11, fontWeight:800, color:D.blue, textTransform:'uppercase', letterSpacing:.7, marginBottom:10 }}>
                  📊 Code-barres principal
                </div>
                <Inp label="Code-barres" value={form.barcode||''} onChange={e=>set('barcode',e.target.value)}
                  placeholder="Scannez ou saisissez..." color={D.blue}/>
              </G>

              <G style={{ padding:14 }} ac={D.indigo}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                  <div style={{ fontSize:11, fontWeight:800, color:D.indigo, textTransform:'uppercase', letterSpacing:.7 }}>
                    📊 Codes-barres supplémentaires ({(form.barcodes||[]).length}/8)
                  </div>
                </div>
                <div style={{ display:'flex', gap:8, marginBottom:12 }}>
                  <input value={newBarcode} onChange={e=>setNewBarcode(e.target.value)}
                    ...
                    style={{ flex:1, background:D.indigoLt, border:`1.5px solid ${D.indigoBd}`, borderRadius:D.Rs,
                      padding:'8px 12px', color:D.txt, fontSize:13, outline:'none' }}/>
                  <button onClick={addBarcode} disabled={(form.barcodes||[]).length>=8}
                    style={{ background:`linear-gradient(135deg,${D.blue},${D.indigo})`, border:'none',
                      borderRadius:D.Rs, padding:'8px 16px', color:'#fff', fontWeight:800,
                      cursor:(form.barcodes||[]).length>=8?'not-allowed':'pointer', fontSize:12,
                      opacity:(form.barcodes||[]).length>=8?.5:1 }}>+ Ajouter</button>
                </div>
                {(form.barcodes||[]).length===0 ? (
                  <div style={{ textAlign:'center', padding:'14px', color:D.muted, fontSize:13 }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                    {(form.barcodes||[]).map((bc,i)=>(
                      <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
                        background:D.indigoLt, border:`1px solid ${D.indigoBd}`, borderRadius:D.Rs,
                        padding:'8px 14px' }}>
                        <div>
                        ...
                        <button onClick={()=>removeBarcode(i)} style={{ background:D.redLt,
                          border:`1px solid ${D.redBd}`, borderRadius:7, padding:'4px 10px',
                          color:D.red, cursor:'pointer', fontSize:11, fontWeight:700 }}>✕</button>
                      </div>
                    ))}
                  </div>
                )}
        ...
        {/* Footer */}
        <div style={{ padding:'12px 18px', borderTop:`1px solid ${D.indigoBd}`,
          display:'flex', gap:10, justifyContent:'flex-end', flexShrink:0 }}>
          <button onClick={onClose} style={{ background:'#F1F5F9', border:'none', borderRadius:D.Rs,
            padding:'10px 20px', cursor:'pointer', fontSize:13, color:D.sub, fontWeight:600 }}>Annuler</button>
          <button onClick={save} disabled={saving} style={{
            background:`linear-gradient(135deg,${D.blue},${D.indigo})`,
            border:'none', borderRadius:D.Rs, padding:'10px 24px',
            color:'#fff', fontWeight:900, cursor:saving?'not-allowed':'pointer',
            fontSize:14, boxShadow:`0 4px 14px ${D.blue}40` }}>
...
export default function Products() {
  ...
  const filtered = useMemo(() => {
    ...
    // Filtres avancés
    if(advStock) {
      const match = advStock.match(/^([<>=!]+)\s*(\d+)/);
      if(match) {
        const [,op,val] = match; const n = Number(val);
        if(op==='<')  list=list.filter(p=>p.stock<n);
        if(op==='>')  list=list.filter(p=>p.stock>n);
        if(op==='<=') list=list.filter(p=>p.stock<=n);
        if(op==='>=') list=list.filter(p=>p.stock>=n);
        if(op==='='||op==='==') list=list.filter(p=>p.stock===n);
      }
    }
    if(advPrice) {
      const match = advPrice.match(/^([<>=!]+)\s*(\d+)/);
      if(match) {
        const [,op,val] = match; const n = Number(val);
        if(op==='<')  list=list.filter(p=>p.sellPrice<n);
        if(op==='>')  list=list.filter(p=>p.sellPrice>n);
        if(op==='<=') list=list.filter(p=>p.sellPrice<=n);
        if(op==='>=') list=list.filter(p=>p.sellPrice>=n);
        if(op==='='||op==='==') list=list.filter(p=>p.sellPrice===n);
      }
    }
    ...
    list.sort((a,b) =>
      ...
      sort==='price'   ? b.sellPrice-a.sellPrice :
      sort==='buy'     ? b.buyPrice-a.buyPrice :
      sort==='expiry'  ? (a.expiry||'9999').localeCompare(b.expiry||'9999') :
      sort==='margin'  ? (b.sellPrice-b.buyPrice)-(a.sellPrice-a.buyPrice) : 0
    );
  ...
  return (
    <div style={{ height:'calc(100vh - 48px)', display:'flex', flexDirection:'column',
      background:D.bg, fontFamily:"'Segoe UI',system-ui,sans-serif", overflow:'hidden' }}>
      ...
      {/* ══ BARRE CONTRÔLES ══ */}
      <G style={{ margin:'0 14px 6px', padding:'10px 14px', flexShrink:0 }} ac={D.blue}>
        <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
          ...
          <div style={{ flex:1, minWidth:220, position:'relative' }}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)}
              ...
              style={{ width:'100%', background:D.blueLt, border:`1.5px solid ${D.blueBd}`,
                borderRadius:D.Rs, padding:'8px 12px 8px 34px', color:D.txt, fontSize:13,
                outline:'none', boxSizing:'border-box' }}/>
          </div>

          {/* Tri */}
          <select value={sort} onChange={e=>setSort(e.target.value)}
            style={{ background:D.indigoLt, border:`1.5px solid ${D.indigoBd}`, borderRadius:D.Rs,
              padding:'8px 12px', color:D.indigo, fontSize:12, fontWeight:600, outline:'none' }}>
            {[['name','A→Z'],['stock','Stock ↑'],['price','Prix vente ↓'],['buy','Prix achat ↓'],['margin','Marge ↓'],['expiry','Péremption']].map(([v,l])=>
              <option key={v} value={v}>{l}</option>)}
          </select>

          {/* Filtres avancés */}
          <button onClick={()=>setShowAdv(!showAdv)} style={{
            background:showAdv?D.violet:D.violetLt, border:`1.5px solid ${D.violetBd}`,
            borderRadius:D.Rs, padding:'7px 14px', color:showAdv?'#fff':D.violet,
          ...
          <div style={{ marginLeft:'auto', display:'flex', gap:6 }}>
            ...
            <button onClick={()=>setModal('new')} style={{
              ...
              border:'none', borderRadius:D.Rs, padding:'8px 18px',
              color:'#fff', fontWeight:800, fontSize:13, cursor:'pointer',
              boxShadow:`0 4px 12px ${D.blue}40`,
            }}>+ Ajouter produit</button>
            <button onClick={()=>setView(v=>v==='liste'?'inventaire':'liste')} style={{
              background:view==='inventaire'?D.amber:D.amberLt,
              border:`1.5px solid ${D.amberBd}`, borderRadius:D.Rs, padding:'8px 14px',
              color:view==='inventaire'?'#fff':D.amber, fontSize:12, fontWeight:700, cursor:'pointer',
            }}>📊 {view==='inventaire'?'Vue liste':'Inventaire'}</button>
          </div>
        </div>

        {/* Filtres avancés panel */}
        {showAdv && (
          <div style={{ marginTop:10, padding:12, background:D.violetLt,
            border:`1px solid ${D.violetBd}`, borderRadius:D.Rs,
            display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
            <Inp label="Filtre stock (ex: <5, >=10)" value={advStock} onChange={e=>setAdvStock(e.target.value)}
              placeholder="ex: <5" color={D.amber}/>
            <Inp label="Filtre prix vente (ex: >500)" value={advPrice} onChange={e=>setAdvPrice(e.target.value)}
              placeholder="ex: >500" color={D.blue}/>
            <Sel label="Catégorie" value={catFilter} onChange={e=>setCatFilter(e.target.value)} color={D.violet}>
              <option value="">Toutes catégories</option>
              {cats.map(c=><option key={c} value={c}>{c}</option>)}
            </Sel>
            <div style={{ display:'flex', gap:6, alignItems:'flex-end' }}>
              <button onClick={()=>{setAdvStock('');setAdvPrice('');setCatFilter('');}} style={{
                flex:1, background:D.redLt, border:`1px solid ${D.redBd}`, borderRadius:D.Rs,
                padding:'8px', color:D.red, fontSize:12, fontWeight:700, cursor:'pointer',
        ...
        <div style={{ display:'flex', gap:6, marginTop:8, flexWrap:'wrap' }}>
          {['tous','favoris','stock bas','rupture','périmés'].map(f=>(
            <Pill key={f} label={f==='tous'?`Tous (${products.length})`:f} active={filter===f}
              onClick={()=>setFilter(f)}
              color={f==='rupture'?D.red:f==='stock bas'?D.amber:f==='périmés'?D.pink:D.blue}/>
          ))}
          {cats.slice(0,6).map(c=>(
            <Pill key={c} label={`${c} (${products.filter(p=>(p.category||'Divers')===c).length})`}
              active={catFilter===c} onClick={()=>setCatFilter(catFilter===c?'':c)} color={D.violet}/>
          ))}
        ...
        <div style={{ flex:1, overflow:'auto', padding:'0 14px 14px' }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
            {Object.entries(inventory.byCat).sort((a,b)=>b[1].valVente-a[1].valVente).map(([cat,data])=>(
              <G key={cat} style={{ padding:16 }} ac={D.blue}>
                ...
                <div style={{ marginTop:8, background:D.indigoLt, borderRadius:6, height:6, overflow:'hidden' }}>
                  <div style={{ height:'100%', background:`linear-gradient(90deg,${D.blue},${D.indigo})`, borderRadius:6,
                    width:`${inventory.valVente>0?Math.min(100,(data.valVente/inventory.valVente)*100):0}%`,
                    transition:'width .5s' }}/>
                </div>
      ...
      {/* ══ TABLEAU PRODUITS ══ */}
      {view==='liste' && (
        <G style={{ flex:1, margin:'0 14px 10px', display:'flex', flexDirection:'column', overflow:'hidden' }} ac={D.blue}>
          {/* En-têtes */}
          <div style={{ flexShrink:0 }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr style={{ background:`linear-gradient(135deg,${D.blue}18,${D.indigo}12)` }}>
                  {['','Réf','Code-barres','Produit','Catégorie','Prix achat','Prix vente','Marge','Stock','Péremption','Statut','Actions'].map(h=>(
                    <th key={h} style={{ padding:'10px 12px', fontSize:10, fontWeight:800,
                      color:D.indigo, textTransform:'uppercase', letterSpacing:.7,
                      textAlign:'left', whiteSpace:'nowrap',
                      borderBottom:`2px solid ${D.indigoBd}` }}>{h}</th>
                  ))}
                </tr>
              </thead>
            </table>
          ...
          <div style={{ flex:1, overflowY:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <tbody>
                {filtered.map((p,i) => {
                  ...
                  return (
                    <tr key={p.id}
                      ...
                      style={{ borderBottom:`1px solid ${D.indigoLt}`, cursor:'pointer',
                        background:i%2===0?D.white:'rgba(238,242,255,0.5)',
                        transition:'background .1s' }}
                      onMouseEnter={e=>e.currentTarget.style.background=D.indigoLt}
                      onMouseLeave={e=>e.currentTarget.style.background=i%2===0?D.white:'rgba(238,242,255,0.5)'}>
                      ...
                      <td style={{ padding:'9px 12px' }}>
                        <span style={{ background:margin>30?D.greenLt:margin>10?D.amberLt:D.redLt,
                          color:margin>30?D.green:margin>10?D.amber:D.red,
                          borderRadius:20, padding:'2px 9px', fontSize:11, fontWeight:700 }}>
                          {margin>0?'+':''}{margin}%
                        </span>
                      </td>
                      <td style={{ padding:'9px 12px', fontWeight:800,
                        color:out?D.red:low?D.amber:D.green, fontSize:13 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                          <button onClick={e=>{e.stopPropagation();adjustStock(p.id,-1);}}
                            style={{ width:22,height:22,borderRadius:6,border:`1px solid ${D.redBd}`,
                              background:D.redLt,cursor:'pointer',color:D.red,fontSize:13,lineHeight:1,display:'flex',alignItems:'center',justifyContent:'center' }}>−</button>
                          <span>{p.stock} {p.unit}</span>
                          <button onClick={e=>{e.stopPropagation();adjustStock(p.id,+1);}}
                            style={{ width:22,height:22,borderRadius:6,border:`1px solid ${D.greenBd}`,
                              background:D.greenLt,cursor:'pointer',color:D.green,fontSize:13,lineHeight:1,display:'flex',alignItems:'center',justifyContent:'center' }}>+</button>
                        </div>
                      </td>
                      <td style={{ padding:'9px 12px', fontSize:11, color:exp?D.red:D.sub, fontWeight:exp?700:400 }}>
                        {p.expiry||'—'}{exp&&' ⚠'}
                      </td>
                      <td style={{ padding:'9px 12px' }}>
                        <span style={{ background:out?D.redLt:low?D.amberLt:D.greenLt,
                          color:out?D.red:low?D.amber:D.green,
                          border:`1px solid ${out?D.redBd:low?D.amberBd:D.greenBd}`,
                      ...
                      <td style={{ padding:'9px 12px' }}>
                        <div style={{ display:'flex', gap:5 }}>
                          <button onClick={e=>{e.stopPropagation();setModal(p);}}
                            style={{ background:D.indigoLt,border:`1px solid ${D.indigoBd}`,
                              borderRadius:7,padding:'4px 10px',color:D.indigo,cursor:'pointer',fontSize:11,fontWeight:700 }}>✏️</button>
                          <button onClick={e=>{e.stopPropagation();setConfirm(p.id);}}
                            style={{ background:D.redLt,border:`1px solid ${D.redBd}`,
                              borderRadius:7,padding:'4px 10px',color:D.red,cursor:'pointer',fontSize:11,fontWeight:700 }}>🗑</button>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                {filtered.length===0&&(
                  <tr><td colSpan={12} style={{ padding:40, textAlign:'center', color:D.muted, fontSize:14 }}>
      ...
      {confirm && (
        <div style={{ position:'fixed',inset:0,background:'rgba(239,68,68,0.1)',backdropFilter:'blur(4px)',
          zIndex:600,display:'flex',alignItems:'center',justifyContent:'center' }}>
          <G style={{ padding:24, width:360, textAlign:'center' }} ac={D.red}>
