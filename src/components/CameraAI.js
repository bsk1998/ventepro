...
// ── Appel Groq Vision (llama-4 scout supporte les images) ────────────────────
...
Réponds en JSON avec cette structure exacte:
{
  ...
  "category": "catégorie probable (Lubrifiants/Filtres/Électrique/Liquides/Distribution/Freinage/Carrosserie/Pneumatiques/Divers)",
  "description": "description courte du produit",
  "confidence": "élevée/moyenne/faible",
  "similarInStock": [
    {"name": "nom du produit similaire dans le stock", "reason": "pourquoi c'est similaire"}
  ],
  "suggestedUnit": "pce/L/kg/m/boîte",
  "suggestedRef": "référence courte suggérée ex: BG-001"
}`;
  ...
  function useResult() {
    ...
    onFound({
      ...
      unit:     result.suggestedUnit || 'pce',
      ref:      result.suggestedRef || '',
      buyPrice: '',
      sellPrice: '',
      stock:    '',
      minStock: 5,
  ...
  return (
    <div>
      <div style={{ fontWeight:800, fontSize:16, color:C.accent, marginBottom:4 }}>
        🔍 Reconnaissance de produit
      </div>
      <div style={{ color:C.sub, fontSize:13, marginBottom:20 }}>
        Photographiez un produit — l'IA l'identifie et cherche des similaires dans votre stock.
      </div>
        ...
        <div>
          {!imagePreview ? (
            <div
              ...
              style={{ border:`2px dashed ${C.accent}50`, borderRadius:16, padding:48,
                textAlign:'center', cursor:'pointer', background:C.accentLo,
                transition:'all .2s' }}
              ...
              <div style={{ display:'flex', gap:10 }}>
                <button onClick={() => { setImagePreview(null); setImageFile(null); setStep('capture'); }}
                  style={{ flex:1, background:C.card, border:`1px solid ${C.border}`, borderRadius:10,
                    padding:'10px', color:C.sub, cursor:'pointer', fontSize:13 }}>
                  🔄 Changer
                </button>
                <button onClick={analyze}
                  style={{ flex:2, background:C.accentGrad||C.accent, border:'none', borderRadius:10,
                    padding:'10px', color:'#000', fontWeight:800, cursor:'pointer', fontSize:14 }}>
                  🔍 Analyser avec l'IA
                </button>
              </div>
            </div>
          ...
          <input ref={fileRef} type="file" accept="image/*" capture="environment"
            ...
            style={{ display:'none' }} />
          {error && (
            <div style={{ marginTop:12, background:C.redLo, border:`1px solid ${C.red}30`,
      ...
      {step === 'analyzing' && (
        <div style={{ textAlign:'center', padding:48 }}>
          ...
          <div style={{ color:C.accent, fontWeight:700, fontSize:16, marginBottom:8 }}>
            Analyse en cours...
          </div>
      ...
      {step === 'result' && result && (
        <div>
          <div style={{ background:C.greenLo, border:`1px solid ${C.green}30`,
            borderRadius:12, padding:16, marginBottom:16 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
              <div>
                <div style={{ color:C.green, fontSize:11, fontWeight:700, textTransform:'uppercase',
                  letterSpacing:.8, marginBottom:4 }}>Produit identifié</div>
              ...
              <span style={{ background:result.confidence==='élevée'?C.greenLo:C.amberLo,
                color:result.confidence==='élevée'?C.green:C.amber,
                border:`1px solid ${result.confidence==='élevée'?C.green:C.amber}30`,
            ...
            <div style={{ marginTop:10, display:'flex', gap:8 }}>
              <span style={{ background:C.blueLo, color:C.blue, border:`1px solid ${C.blue}30`,
                borderRadius:20, padding:'2px 10px', fontSize:11, fontWeight:700 }}>
                {result.category}
              </span>
              <span style={{ background:C.card, color:C.sub, borderRadius:20,
                padding:'2px 10px', fontSize:11 }}>
                Unité: {result.suggestedUnit}
              </span>
            </div>
          </div>
          ...
          {result.similarInStock?.length > 0 && (
            <div style={{ marginBottom:16 }}>
              <div style={{ fontWeight:700, fontSize:13, marginBottom:10, color:C.amber }}>
                🔗 Produits similaires dans votre stock :
              </div>
              {result.similarInStock.map((s, i) => (
                <div key={i} style={{ background:C.amberLo, border:`1px solid ${C.amber}25`,
          ...
          {result.similarInStock?.length === 0 && (
            <div style={{ background:C.card, border:`1px solid ${C.border}`,
              borderRadius:10, padding:'12px 14px', marginBottom:16, color:C.sub, fontSize:13 }}>
          ...
          <div style={{ display:'flex', gap:10 }}>
            ...
            <button onClick={useResult}
              style={{ flex:2, background:C.accent, border:'none', borderRadius:10,
                padding:'10px', color:'#000', fontWeight:800, cursor:'pointer', fontSize:13 }}>
  ...
  "products": [
    {
      ...
      "ref": "référence si visible, sinon ''",
      "quantity": 1,
      "unit": "pce/L/kg/m/boîte",
      "buyPrice": 0,
      "category": "catégorie probable parmi: Lubrifiants/Filtres/Électrique/Liquides/Distribution/Freinage/Carrosserie/Pneumatiques/Divers"
    }
  ]
}

Important: buyPrice = prix unitaire d'achat. Si pas visible, mets 0. Ne mets JAMAIS de prix de vente.`;

      const raw    = await analyzeImage(base64, mime, system, user);
      const cleaned = raw.replace(/```json|```/g, '').trim();
      const parsed  = JSON.parse(cleaned);

      // Préparer les produits avec sellPrice vide
      const prods = (parsed.products || []).map((p, i) => ({
        ...
        sellPrice: '', // à remplir par l'utilisateur
        stock:     p.quantity || 1,
        minStock:  5,
        expiry:    '',
        favorite:  false,
        barcode:   '',
        _supplier: parsed.supplier,
        _invoiceDate: parsed.invoiceDate,
      }));
      ...
      setError(e.message.includes('JSON')
        ? 'Impossible de lire le bon. Essayez avec une photo plus nette et bien éclairée.'
        : e.message);
      setStep('preview');
    }
  }
  ...
  return (
    <div>
      <div style={{ fontWeight:800, fontSize:16, color:C.purple, marginBottom:4 }}>
        📄 Scanner un bon d'achat
      </div>
      <div style={{ color:C.sub, fontSize:13, marginBottom:20 }}>
        Photographiez votre bon de livraison — l'IA ajoute automatiquement les produits avec les prix d'achat. Vous renseignez uniquement les prix de vente.
      </div>
        ...
        <div>
          {!imagePreview ? (
            <div onClick={() => fileRef.current.click()}
              style={{ border:`2px dashed ${C.purple}50`, borderRadius:16, padding:48,
                textAlign:'center', cursor:'pointer', background:C.purpleLo, transition:'all .2s' }}
            ...
            <div>
              ...
              <div style={{ display:'flex', gap:10 }}>
                ...
                <button onClick={analyze}
                  style={{ flex:2, background:`linear-gradient(135deg,${C.purple},${C.violet})`,
                    border:'none', borderRadius:10, padding:'10px', color:'#fff',
                    fontWeight:800, cursor:'pointer', fontSize:14 }}>
                  📖 Lire le bon avec l'IA
                </button>
              </div>
            </div>
          )}
      ...
      {step === 'review' && (
        <div>
          <div style={{ background:C.greenLo, border:`1px solid ${C.green}30`,
            ...
            {products[0]?._supplier && (
              <span style={{ color:C.sub, fontSize:12 }}>
                Fournisseur: {products[0]._supplier}
              </span>
            )}
          </div>

          {/* Légende */}
          <div style={{ background:C.amberLo, border:`1px solid ${C.amber}30`,
            borderRadius:10, padding:'10px 14px', marginBottom:14,
            display:'flex', alignItems:'center', gap:8 }}>
            <span style={{ fontSize:18 }}>✏️</span>
            <span style={{ color:C.amber, fontSize:12, fontWeight:600 }}>
              Remplissez les <strong>prix de vente</strong> avant d'enregistrer. Les autres champs sont modifiables.
            </span>
          </div>
              ...
              <div key={p._id} style={{ background:C.card, border:`1px solid ${C.border}`,
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                  <input
                    ...
                    style={{ flex:1, background:C.bg, border:`1px solid ${C.border}`, borderRadius:8,
                      padding:'7px 10px', color:C.text, fontSize:14, fontWeight:700, outline:'none' }}
                  />
                  <button onClick={() => removeProduct(p._id)}
                    style={{ marginLeft:8, background:C.redLo, border:'none', borderRadius:8,
                      padding:'6px 10px', color:C.red, cursor:'pointer', fontSize:13 }}>✕</button>
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:8 }}>
                  <div>
                    ...
                    <input type="number" value={p.buyPrice}
                      onChange={e => updateProduct(p._id, 'buyPrice', e.target.value)}
                      style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, borderRadius:8,
                        padding:'7px 8px', color:C.sub, fontSize:13, outline:'none' }}
                    />
                  </div>
                  <div>
                    <div style={{ color:C.amber, fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:.6, marginBottom:4 }}>
                      Prix vente ✏️
                    </div>
                    <input type="number" value={p.sellPrice} placeholder="À définir"
                      onChange={e => updateProduct(p._id, 'sellPrice', e.target.value)}
                      style={{ width:'100%', background:C.bg,
                        border:`2px solid ${p.sellPrice ? C.green+'60' : C.amber+'60'}`,
                        borderRadius:8, padding:'7px 8px', color:C.text, fontSize:13,
                  ...
                  <div>
                    ...
                    <input type="number" value={p.stock}
                      onChange={e => updateProduct(p._id, 'stock', e.target.value)}
                      style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, borderRadius:8,
                        padding:'7px 8px', color:C.sub, fontSize:13, outline:'none' }}
                    />
                  </div>
                  <div>
                    ...
                    <select value={p.category} onChange={e => updateProduct(p._id, 'category', e.target.value)}
                      style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, borderRadius:8,
                        padding:'7px 6px', color:C.sub, fontSize:12, outline:'none' }}>
                      {['Lubrifiants','Filtres','Électrique','Liquides','Distribution','Freinage','Carrosserie','Pneumatiques','Divers'].map(c =>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={() => { setStep('capture'); setImagePreview(null); setProducts([]); }}
              style={{ flex:1, background:C.card, border:`1px solid ${C.border}`, borderRadius:10,
                padding:'11px', color:C.sub, cursor:'pointer', fontSize:13 }}>
              🔄 Rescanner
            </button>
            <button onClick={saveAll} disabled={saving || products.length === 0}
              style={{ flex:2, background:`linear-gradient(135deg,${C.green},${C.accent})`,
                border:'none', borderRadius:10, padding:'11px', color:'#000',
                fontWeight:900, cursor:saving?'not-allowed':'pointer', fontSize:14,
                opacity: saving ? .6 : 1 }}>
...
export default function CameraAI({ onClose, onProductFound, onProductsImported, existingProducts = [] }) {
  const [mode, setMode] = useState(null); // null | 'recognize' | 'invoice'

  return (
    <div style={{ position:'fixed', inset:0, background:'#000000aa', zIndex:250,
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:24,
        ...
        boxShadow:'0 40px 100px #000d' }}>

        {/* Header */}
        <div style={{ padding:'20px 24px', borderBottom:`1px solid ${C.border}`,
          display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <div style={{ fontWeight:900, fontSize:17, fontFamily:C.fontDisplay }}>📸 IA Caméra</div>
          ...
          {!mode && (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
              <button onClick={() => setMode('recognize')}
                style={{ background:C.accentLo, border:`2px solid ${C.accent}40`,
                  borderRadius:16, padding:28, cursor:'pointer', textAlign:'center',
                  transition:'all .2s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor=C.accent; e.currentTarget.style.background=C.accentLo; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor=C.accent+'40'; }}>
                <div style={{ fontSize:44, marginBottom:12 }}>🔍</div>
                <div style={{ fontWeight:800, fontSize:15, color:C.accent, marginBottom:6 }}>
                  Reconnaître un produit
                </div>
                <div style={{ color:C.sub, fontSize:12, lineHeight:1.5 }}>
                  Photographiez un article et l'IA l'identifie automatiquement et cherche des similaires dans votre stock
                </div>
              </button>

              <button onClick={() => setMode('invoice')}
                style={{ background:C.purpleLo, border:`2px solid ${C.purple}40`,
                  borderRadius:16, padding:28, cursor:'pointer', textAlign:'center',
                  transition:'all .2s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor=C.purple; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor=C.purple+'40'; }}>
                <div style={{ fontSize:44, marginBottom:12 }}>📄</div>
                <div style={{ fontWeight:800, fontSize:15, color:C.purple, marginBottom:6 }}>
                  Scanner un bon d'achat
                </div>
                <div style={{ color:C.sub, fontSize:12, lineHeight:1.5 }}>
                  Photographiez un bon de livraison — l'IA extrait et ajoute tous les produits automatiquement
                </div>
              </button>
            </div>
          )}
